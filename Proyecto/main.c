...
#define GRAPHICS_COLOR_BRIGHT_GREEN   0x007FFF00   // verde muy vivo (R=127, G=255, B=0)

Graphics_Context g_sContext;

// Modo accesible: 0 = colores, 1 = símbolos
...
uint16_t lfsr_siguiente(void){
    uint16_t bit = ((lfsr >> 0) ^ (lfsr >> 2) ^ (lfsr >> 3) ^ (lfsr >> 5)) & 1; // Calculamos un nuevo bit como el XOR de varios taps
...
// Funcion para dibujar una estrella simple (pequeña o grande)
...
int main(void){
    ...
    unsigned int c  = 26;  // Tamaño del hueco central
    ...
    typedef enum { BIENVENIDA=0, MENSAJE_RONDA, MAQUINA, TURNO_JUGADOR, VICTORIA, VICTORIA_FINAL, FIN } estados_t;
        ...
        if(tick){
            ...
            case BIENVENIDA:
                if(tms == 1){
                    ...
                    Graphics_drawStringCentered(&g_sContext, "SIMON DICE", -1, 64, 50, TRANSPARENT_TEXT); // Dibujamos texto de bienvenida
                ...
                if(start){
                    ...
                    // Si a la vez que pulsamos el boton del joystick lo movemos a la izquierda, entraremos en el modo de simbolos
                    ejex = (unsigned int)lee_ch(0);   // Leemos eje X del joystick
                        if(ejex < 100) {
                            modo_simbolos = 1;            // Joystick a la izquierda símbolos
                        }
                        else {
                    ...
                    estado = MENSAJE_RONDA; // Saltamos al estado de mostrar el mensaje de la ronda
                    tms = 0;
                }
                break;
            ...
            case MENSAJE_RONDA:
                ...
                if(tms >= (800/25)){
                    ...
                    for(i=0;i<4;i++){
                        // Si no estamos en el modo de simbolos, dibujamos los rectangulos de color
                        if(!modo_simbolos){
                            Graphics_setForegroundColor(&g_sContext, color_bajo[i]);
                            Graphics_fillRectangle(&g_sContext, &boton[i]);
                        }
                        // Si estamos en el modo de simbolos, dibujamos los simbolos
                        else{
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_BLACK);
                            Graphics_fillRectangle(&g_sContext, &boton[i]);
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_WHITE);
                            Graphics_drawStringCentered(&g_sContext,(int8_t*)simbolos[i],-1,(boton[i].xMin + boton[i].xMax)/2,(boton[i].yMin + boton[i].yMax)/2,TRANSPARENT_TEXT);
                        }
                    }

                    estado = MAQUINA; // Saltamos al estado en el que se reproduce la secuencia que se debe memorizar
                    tms = 0;
                }
                break;

            case MAQUINA:
                // Si ya se han mostrado todos los pasos de esa ronda, pasamos al turno del jugador
                if(paso_maquina >= ronda){
                    ...
                    paso_jugador = 0;
                    estado_joystick = 0;
                    estado = TURNO_JUGADOR; //  Saltamos al estado en el que el jugador debe reproducir la secuencia
                    tms = 0;
                    break;
                }
                ...
                if(sub_maquina == 0){
                    if(tms == 1){
                        ...
                        unsigned char color = secuencia[paso_maquina]; // Asignamos el color por el que va la secuencia
                        unsigned char color_aux = color - 1; // Array de 0 a 3

                        // Si no estamos en el modo de simbolos, iluminamos el color correspondiente
                        if(!modo_simbolos){
                            Graphics_setForegroundColor(&g_sContext, color_alto[color_aux]);
                            Graphics_fillRectangle(&g_sContext, &boton[color_aux]);
                        }
                        // Si estamos en el modo de simbolos, iluminamos el simbolo correspondiente
                        else{
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_WHITE);
                            Graphics_fillRectangle(&g_sContext, &boton[color_aux]);
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_BLACK);
                            Graphics_drawStringCentered(&g_sContext,(int8_t*)simbolos[color_aux],-1,(boton[color_aux].xMin + boton[color_aux].xMax)/2,(boton[color_aux].yMin + boton[color_aux].yMax)/2,TRANSPARENT_TEXT);
                        }

                        sonido_color(color); // Suena el sonido correspondiente del color seleccionado
                        ...
                        unsigned int W = interior.xMax - interior.xMin; // Tamaño de la barra de progreso
                        unsigned int relleno = (unsigned int)((unsigned long)W * (paso_maquina+1) / ronda); // Tamaño del relleno actual
                    ...
                    if(tms >= T){
                        ...
                        // Si no estamos en el modo de simbolos, apagamos el color iluminado
                        if(!modo_simbolos){
                            Graphics_setForegroundColor(&g_sContext, color_bajo[color_aux]);
                            Graphics_fillRectangle(&g_sContext, &boton[color_aux]);
                        }
                        // Si estamos en el modo de simbolos, apagamos el simbolo iluminado
                        else {
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_BLACK);
                            Graphics_fillRectangle(&g_sContext, &boton[color_aux]);
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_WHITE);
                    ...
                    if(tms >= T4){
                        paso_maquina++; // Si pasa el tiempo de pausa, pasamos al siguiente paso de la secuencia
                        sub_maquina = 0; // Volvemos al subestado de iluminar el color
                        tms = 0;
                    }
                }
                break;
            ...
            case TURNO_JUGADOR: {
                // Si se acaba el tiempo para seleccionar el color, saltamos al estado de game over
                if(tms >= T2){
                    estado = FIN;
                    tms = 0;
                    apaga_sonido();
                ...
                if(tms == 1){
                    // Inicializamos los botones según el modo
                    for(i=0; i<4; i++){
                        // Si estamos en el modo de simbolos, inicializamos los simbolos apagados
                        if(modo_simbolos){
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_BLACK); // fondo negro
                            Graphics_fillRectangle(&g_sContext, &boton[i]);
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_WHITE); // letra blanca
                            Graphics_drawStringCentered(&g_sContext,(int8_t*)simbolos[i],-1,(boton[i].xMin + boton[i].xMax)/2,(boton[i].yMin + boton[i].yMax)/2,TRANSPARENT_TEXT);
                        }
                        // Si no estamos en el modo de simbolos, inicializamos los rectangulos de color apagado
                        else{
                            Graphics_setForegroundColor(&g_sContext, color_bajo[i]);
                            Graphics_fillRectangle(&g_sContext, &boton[i]);
                        }
                ...
                // Variables para leer la posicion del joystick
                ejex = (unsigned int)lee_ch(0);
                ejey = (unsigned int)lee_ch(3);

                //Desplazamiento respecto al centro del joystick
                int dx = (int)ejex - 512;
                int dy = (int)ejey - 512;

                // Dejamos una zona muerta en el medio para que no se elija un color por error
                ...
                if(estado_joystick == 0){
                    // Si salimos de la zona muerta, registramos el color
                    if(!(dx < 100 && dx > -100 && dy < 100 && dy > -100)) {
                        estado_joystick = 1;

                        // Comparamos si el joystick se esta moviendo mas en el eje x o en el eje y
                        int adx = (dx<0)?-dx:dx;
                        int ady = (dy<0)?-dy:dy;
                        ...
                        // Si el joystick se mueve en el eje y
                        else {
                            if(dy > 0) {
                                elegido = 1;  // Arriba
                            }
                ...
                if(elegido != 0) {
...

                    {
                        ...
                        // Si no estamos en el modo de simbolos, se ilumina automaticamente el color seleccionado
                        if(!modo_simbolos) {
                            Graphics_setForegroundColor(&g_sContext, color_alto[color_aux]);
                            Graphics_fillRectangle(&g_sContext, &boton[color_aux]);
                        }
                        // Si estamos en el modo de simbolos, se ilumina automaticamente el simbolo seleccionado
                        else {
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_WHITE);
                            Graphics_fillRectangle(&g_sContext, &boton[color_aux]);
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_BLACK);
                        ...
                        sonido_color(elegido); // Suena la nota musical correspondiente al color o simbolo elegido
                        seleccion = elegido;
                        tflash = 0;
                    }
                    ...
                    if(elegido == esperado){
                        puntuacion++; // Aumentamos en 1 la puntuacion
                        paso_jugador++; // Pasamos al siguiente paso de la ronda
                        ...
                        unsigned int W = interior.xMax - interior.xMin; // Tamaño de la barra de progreso
                        unsigned int relleno = (unsigned int)((unsigned long)W * (paso_jugador) / ronda); // Tamaño del relleno
                        // Dibujamos el relleno
                        if(relleno > 0){
                            Graphics_Rectangle f = interior;
                            f.xMax = f.xMin + relleno;
                        ...
                        // Si el jugador completa todos los pasos correctamente, se pasa al estado de ronda completada
                    ...
                    // Si el color elegido es incorrecto, saltamos al estado de game over
                    else {
                        estado = FIN;
                        tms = 0;
                    }
                }
                // Si se ha seleccionado un color o simbolo
                if(seleccion != 0){
                    tflash++; // Aumentamos el tiempo auxiliar de la iluminacion
                    if(tflash >= (T/2)) {
                        unsigned char color_aux = seleccion - 1;
                        // Tras seleccionar un color, este se apaga tras T/2
                        if(!modo_simbolos) {
                            Graphics_setForegroundColor(&g_sContext, color_bajo[color_aux]);
                            Graphics_fillRectangle(&g_sContext, &boton[color_aux]);
                        }
                        // Tras seleccionar un simbolo, este se apaga tras T/2
                        else {
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_BLACK); // fondo negro
                            Graphics_fillRectangle(&g_sContext, &boton[color_aux]);
                            Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_WHITE); // letra blanca
            ...
            case VICTORIA:
                if(tms == 1) {
                    Graphics_clearDisplay(&g_sContext);
                    // Pintamos el mensaje de ronda superada
                    Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_WHITE);
                    ...
                    // Inicializamos el marco y el dibujo de la animacion
                ...
                t_vict++; // Aumentamos el tiempo auxiliar de victoria para temporizar las notas de la musica de ronda superada

                // Secuencia de notas de la musica de victoria (pares sonido, impares silencio)
                if(paso_vict == 0) {
                    suena_hz(DO);
                }
                if(paso_vict == 1) {
                ...
                // Si el paso es par (sonido), dura 150 ms
                if((paso_vict % 2) == 0){
                    if(t_vict >= 6) {
                        t_vict = 0;
                        paso_vict++;
                ...
                // Si el paso es impar (silencio), dura 50 ms
                else {
                    if(t_vict >= 2) {
                        t_vict = 0;
                        paso_vict++;
                ...
                // Animacion cada 50 ms
                if((tms % 2) == 0){
                    // Borramos las barras anteriores del ecualizador para poder dibujar unas nuevas
                    Graphics_setForegroundColor(&g_sContext, GRAPHICS_COLOR_BLACK);
                    Graphics_fillRectangle(&g_sContext, &barra1);
                    Graphics_fillRectangle(&g_sContext, &barra2);
                    Graphics_fillRectangle(&g_sContext, &barra3);

                    unsigned char h1=8, h2=18, h3=12; // Alturas iniciales de las barras del ecualizador
                    // 4 patrones de alturas segun el instante de la animacion
                    if(anim==0) {
                        h1=8;  h2=18; h3=12;
                    }
                    if(anim==1) {
                    ...
                    // Si es un paso impar (silencio), barras del ecualizador pequeñas
                    if((paso_vict % 2) == 1){
                        h1 = 6; h2 = 6; h3 = 6;
                    }
                    ...
                    // Calculamos las coordenadas de las barras variables segun h1, h2 y h3
                    Graphics_Rectangle b1 = barra1;
                    b1.yMin = (unsigned int)(barra1.yMax - h1);
                    Graphics_Rectangle b2 = barra2;
                    b2.yMin = (unsigned int)(barra2.yMax - h2);
                    Graphics_Rectangle b3 = barra3;
                    b3.yMin = (unsigned int)(barra3.yMax - h3);

                    // Dibujamos las barras del ecualizador
                    Graphics_fillRectangle(&g_sContext, &b1);
                    Graphics_fillRectangle(&g_sContext, &b2);
                    Graphics_fillRectangle(&g_sContext, &b3);

                    anim++; // Sumamos 1 a la variable que segun su valor va cambiando la animacion

                    if(anim >= 4) {
                        anim = 0; // Como tenemos 4 estados posibles de la animacion, cuando llegamos a 5, reseteamos la animacion
                    }
                }

                // Una vez acabe la melodia
                if(paso_vict >= 12){
                    apaga_sonido(); // Buzzer desactivado
            ...
            case VICTORIA_FINAL:
                if(tms == 1){
                    ...
                    for(i=0;i<N_ESTRELLAS;i++){
                        ...
                        if((lfsr_siguiente() & 1) == 0){
                            star_y[i] = (unsigned char)(8 + (lfsr_siguiente() % 22));
                        }else{
                            star_y[i] = (unsigned char)(85 + (lfsr_siguiente() % 35));
                        }
                ...
                if(t_victf == 1){
                    if(melodia_victf[paso_victf] == 0){
                        apaga_sonido();
                    }
                    else{
                        suena_hz(melodia_victf[paso_victf]);
                    }
                }
            ...
            case FIN:
                if(tms == 1) {
                    ...
                    // Pintamos el mensaje de pulsar start para comenzar otra partida
                    Graphics_drawStringCentered(&g_sContext, "PULSA START", -1, 64, 95, TRANSPARENT_TEXT);

                    suena_hz(150); // Suena el sonido de game over
                }
                // Tras un tiempo, apagamos el sonido de derrota
                if(tms >= (600/25)) {
                    apaga_sonido();
                }
                // Si pulsamos start, volvemos a la pantalla de bienvenida
                if(start) {
                    start = 0;
                    estado = BIENVENIDA;
                    tms = 0;
